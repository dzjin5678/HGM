...
class HeteroRGCNLayer(nn.Module):
    def __init__(self, in_feats, out_feats, etypes, activation=F.relu, weighted=False, bias=False,
        ...
        self.gcn_layers = nn.ModuleDict({
                etype: HeteroGraphConv(in_feats=in_feats, out_feats=out_feats, activation=activation,
...
class HeteroGraphCLF(nn.Module):
    def __init__(self, in_dim, hidden_dims, etypes, dropout=0.0, activation=F.relu,
                 weighted=False, bias=False, residual=True, meta_paths=None):
        ...
        self.classify_layer = nn.Sequential(nn.Linear(246, 2))
        ...
        for step, ntype in enumerate(self.ntypes):
            self.ntypes_meta_path_emb[ntype] = nn.ModuleDict({
                meta_path_name: GCNLayer(in_feats=in_dim, out_feats=hidden_dims[-1], activation=activation,
                                         weighted=False, bias=bias, norm=True, residual=residual,
    ...
    def encode(self, g, method='mean'):
        ...
        feat = self.readout(g, method)
        return feat.cpu().numpy()

    def readout(self, bg, method='mean'):
        bs = bg.batch_size
        l_feat = bg.ndata['h']['l']
        r_feat = bg.ndata['h']['r']

        if method == 'mean':
            l_feat, r_feat = th.mean(l_feat, 1), th.mean(r_feat, 1)
        else:
            l_feat, r_feat = self.readout_layer(l_feat), self.readout_layer(r_feat)
        l_feat = l_feat.reshape(bs, -1)
        r_feat = l_feat.reshape(bs, -1)

        feat = th.cat([l_feat, r_feat], 1)
        return feat
